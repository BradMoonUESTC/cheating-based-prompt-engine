...
class ProjectTaskMgr(object):
    ...
    def update_result_title(self, id, result_title):
        self._operate_in_session(self._update_result_title, id, result_title)
    def _update_result_title(self, session, id, result_title):
        session.query(Project_Task).filter_by(id=id).update({Project_Task.title:result_title})
        session.commit()
