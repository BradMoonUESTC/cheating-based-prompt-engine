    ...
    OwnableUpgradeable
{
    ...
    function _validateMaxGlobalSize(
        ...
        (, , , uint256 guaranteedEth, uint256 globalShortSize, , ) = IVault(
            vault
        ).getPoolInfo(_indexToken);
        if (_isLong) {
            uint256 maxGlobalLongSize = maxGlobalLongSizes[_indexToken];
            if (
                maxGlobalLongSize > 0 &&
                guaranteedEth + _sizeDelta > maxGlobalLongSize
            ) {
            ...
            if (
                ...
                globalShortSize + _sizeDelta > maxGlobalShortSize
            ) {
    ...
    function _collectFees(
        ...
        if (shouldDeductFee) {
            uint256 afterFeeAmount = (_amountIn *
                (Constants.PERCENTAGE_FACTOR - depositFee)) /
