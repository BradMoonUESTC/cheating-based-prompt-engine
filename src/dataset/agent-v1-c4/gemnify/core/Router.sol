...
contract Router is IRouter, OwnableUpgradeable {
    ...
    function directPoolDeposit(address _token, uint256 _amount) external {
        IERC20Upgradeable(_token).safeTransferFrom(_sender(), vault, _amount);
    ...
    function increasePosition(
        ...
        if (_path.length > 1 && _amountIn > 0) {
            uint256 amountOut = _swap(_path, _minOut, address(this));
            IERC20Upgradeable(_path[_path.length - 1]).safeTransfer(
                vault,
                amountOut
            );
        }
    ...
    function decreasePositionAndSwap(
        ...
        IERC20Upgradeable(_path[0]).safeTransfer(vault, amount);
    ...
    function _transferETHToVault() private {
        ...
        IERC20Upgradeable(weth).safeTransfer(vault, msg.value);
    }
