...
import {BasePositionManager, MathUpgradeable, SafeERC20Upgradeable, IERC20Upgradeable, IRouter, IVault, IReferralStorage} from "./BasePositionManager.sol";
...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    function executeIncreasePosition(
        ...
        if (request.amountIn > 0) {
            ...
            IERC20Upgradeable(request.path[request.path.length - 1])
                .safeTransfer(vault, afterFeeAmount);
        }
    ...
    function executeDecreasePosition(
        ...
        if (amountOut > 0) {
            ...
            if (request.withdrawETH) {
                ...
                IERC20Upgradeable(request.path[request.path.length - 1])
                    .safeTransfer(request.receiver, amountOut);
            }
        }
    ...
    function _callRequestCallback(
        ...
        if (!AddressUpgradeable.isContract(_callbackTarget)) {
            return;
        }
